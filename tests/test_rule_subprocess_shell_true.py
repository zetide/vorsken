"""
Integration test for the `subprocess-shell-true` Semgrep rule.

Runs Semgrep against tests/fixtures/subprocess_shell_sample.py and asserts:
- the rule fires on every shell=True call (run / Popen / call / check_call /
  check_output)
- the rule does NOT fire on safe (list-argument / shell=False) calls
...
RULE_PATH = REPO_ROOT / "rules" / "custom" / "subprocess_injection.yml"
FIXTURE_PATH = REPO_ROOT / "tests" / "fixtures" / "subprocess_shell_sample.py"
...
SAFE_MARKERS = ["SAFE-LIST", "SAFE-SHELL-FALSE"]
...
def test_rule_fires_on_shell_true(semgrep_findings: list[dict], marker: str) -> None:
    ...
    assert any(label_line < line <= label_line + 3 for line in fired), (
        f"expected {marker} shell=True call to fire near line {label_line}, got {fired}"
    )
