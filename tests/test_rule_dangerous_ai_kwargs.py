"""
Integration test for the `dangerous-ai-kwargs` Semgrep rule.

Runs Semgrep against tests/fixtures/vulnerable_sample.py and asserts:
- the rule fires when trust_remote_code / allow_dangerous_deserialization /
  allow_dangerous_code are literally True
- the rule does NOT fire when the flag is False or omitted
...
RULE_PATH = REPO_ROOT / "rules" / "custom" / "ai-generated" / "dangerous_ai_kwargs.yml"
FIXTURE_PATH = REPO_ROOT / "tests" / "fixtures" / "vulnerable_sample.py"

RULE_ID = "dangerous-ai-kwargs"
...
def test_rule_fires_on_dangerous_kwarg(semgrep_findings: list[dict], marker: str) -> None:
    ...
    assert any(label_line < line <= label_line + 3 for line in fired), (
        f"expected {marker} kwarg=True to fire near line {label_line}, got {fired}"
    )
