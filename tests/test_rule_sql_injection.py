"""
Integration test for the `sql-injection` Semgrep rule.

Runs Semgrep against tests/fixtures/vulnerable_sample.py and asserts:
- the rule fires on every dynamically built query (f-string / concatenation /
  str.format() / % formatting) passed to execute()
- the rule does NOT fire on safe (parameterized / %s-placeholder / static) calls
...
RULE_PATH = REPO_ROOT / "rules" / "custom" / "sql_injection.yml"
...
def test_rule_fires_on_sqli(semgrep_findings: list[dict], marker: str) -> None:
    ...
    assert any(label_line < line <= label_line + 3 for line in fired), (
        f"expected {marker} query to fire near line {label_line}, got {fired}"
    )
